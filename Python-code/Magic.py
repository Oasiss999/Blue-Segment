...
def read_and_create(filename, window):
    if not os.path.exists('Euler_Images'):
            os.makedirs('Euler_Images')
    if not os.path.exists('Chemical_Images'):
            os.makedirs('Chemical_Images')

    
    def show_spinner():
        spinner_window = Toplevel(window)
        spinner_window.configure(bg="white")

        spinner_image = PhotoImage(file='/Users/oasis/Documents/Blue Rocks/build/assets/frame0/spinerGif.gif')
        spinner_label = Label(spinner_window, image=spinner_image, bg="white")
        spinner_label.image = spinner_image
        spinner_label.pack(padx=50, pady=50)

        spinner_window.transient(window)
        spinner_window.grab_set()

        return spinner_window

    def hide_spinner(spinner_window):
        spinner_window.grab_release()
        spinner_window.destroy()

    def process_data(file):
        max_chemicals = find_max_of_chem(file)

        get_phase_color(file)
      ...
      # Convert the arrays to uint8 arrays with values in the range 0-255
        ...
        imageio.imwrite('Chemical_Images/AL_fromFile.png', AL_img_uint8)
        imageio.imwrite('Chemical_Images/CA_fromFile.png', CA_img_uint8)
        imageio.imwrite('Chemical_Images/NA_fromFile.png', NA_img_uint8)
        imageio.imwrite('Chemical_Images/FE_fromFile.png', FE_img_uint8)
        imageio.imwrite('Chemical_Images/SI_fromFile.png', SI_img_uint8)
        imageio.imwrite('Chemical_Images/K_fromFile.png', K_img_uint8)

    spinner_window = show_spinner()

    # Start a separate thread to process data
    processing_thread = threading.Thread(target=process_data, args=(open(filename),))
    processing_thread.start()

    # Hide the spinner once processing is done
    processing_thread.join()
    hide_spinner(spinner_window)
